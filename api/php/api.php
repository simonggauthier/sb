...
class Api
{
	...
	*/
	public function getBook ()
	{
		...
		$book = $this->database->findBook($_GET['name'], $_SESSION['userId']);
		...
		$objectives = $this->database->getAllObjectives($book->id);

		foreach ($objectives as $objective) 
		{
			array_push($ret['objectives'], $objective->toMap());
		}
	...
	*/
	public function createCategory ()
	{
		...
		$objective = new Objective(0, $_POST['bookId'], $category->id, 0);
		$this->database->createObjective($objective);
	...
	*/
	public function deleteCategory ()
	{
		...
		$this->database->deleteObjectiveForCategory($category->id);
	...
	*/
	public function updateObjective ()
	{
		...
		{
			throw new ApiError(new GenericError('Invalid update objective request'), 400);
		}

		$objective = $this->database->getObjective($_POST['id']);

		if (!$objective)
		{
			throw new ApiError(new GenericError('Invalid objective id'), 400);
		}

		$book = $this->database->getBook($objective->bookId);
		...
		$objective->amount = str_replace('.', '', '' . $_POST['amount']);
		$this->database->updateObjective($objective);

		$this->ok($objective->toMap());
	}
	...
	*/
	public function conversionScript ()
	{
		...
		$book = $this->database->findBook('Budget', $_SESSION['userId']);

		$categories = $this->database->getAllCategories($book->id);

		foreach ($categories as $category) {
			$objective = new Objective(-1, $book->id, $category->id, 1000);

			$this->database->createObjective($objective);

			array_push($actions, 'Created objective ' . $objective->id);
		}
...
class Router
{
	...
	private function route () {
		...
		$routes['POST']['objective'] = function () {
			$this->api->updateObjective();
		};
